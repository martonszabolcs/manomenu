...
onCancel() {
  ...
  componentWillMount () {
    ...
    .then((res) => {
      this.setState({
        content: res,
        HTML: '<style> img{width:100%;} </style> <div style="-webkit-user-select: none;">' + res.content + '</div>'
      })
    })
  }
...
pictureHide(){
  ...
  if ( this.state.content.imageUrl != 'http://46.101.62.53/Apps/rest/image/null') {
    ...
    key={Math.random()}
          ...
          style={{width:width, height:height/3, zIndex:100}}/>
          )
  } else {
    return ( <View/>)
  }
...
const HTML = this.state.content.content;
const HTML2 = `
  <div class="comment">
    <span class="c00">
    ...
    return (
      ...
      <TouchableOpacity  onPress={() => { Actions.pop()}}>
        <Image
          ...
          style={{width:31/3, height:58/3, zIndex:100, marginLeft:10, marginRight:10}}/>
      </TouchableOpacity>
            <View>
        ...
        <View style={{flex:1}}>
         <WebView ref={'webview'} source={{html: this.state.HTML}} scalesPageToFit={false}
